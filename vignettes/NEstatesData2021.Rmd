...
neUSsites3 <- get_datasets(datasettype = "diatom", loc = neus, limit = 299, offset = 599)

neUSsites4 <- get_datasets(datasettype = "diatom", loc = neus, limit = 299, offset = 898)
...
plotLeaflet(NE_NLA_sites)

```

```{r plotting another dataset ontop of above}
...
abitibi_sites <- get_sites(abitibi_ids)

#using modified plotLeaflet, found locally in "Custom Functions"
abitibi_form <- map(abitibi_sites@sites, function(x) {
              ...
              bind_rows()
####result siteids > put into format for plotLeaflet
#using modified plotLeaflet, found locally in "Custom Functions"
plotLeaflet(NE_NLA_sites) %>%
  addCircleMarkers(data = abitibi_form, lng = abitibi_form$long, lat = abitibi_form$lat, radius = 3, color = '#0FF',
                             popup = paste0("<b>", abitibi_form$sitename,
                                            "</b><br><b>Description:</b> ",
                                            abitibi_form$description,
