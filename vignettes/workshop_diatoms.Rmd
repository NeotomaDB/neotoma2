...
##### Navigating the structure of a sites object (metadata).

The `uwallf_site` object is composed of several smaller objects of class `site`.  We can call any individual site using `[[ ]]`, placing the index of the desired object between the brackets.  Then we can also call the particular variable we want using the `$` symbol.
...
write.csv(custom_datasets,"C:\\Users\\eco37\\Desktop\\diat_uwallf.csv", row.names = FALSE)
```
...
SIMON and SOCORRO - the following code does not work - site object input is a problem.  Maybe it will work with a different sites object?
...
#to save the output as a CSV file, which could be opened in Microsoft Excel
write.csv(diat_uwallf_collunits,"C:\\Users\\eco37\\Desktop\\collunits_uwallf.csv", row.names = FALSE)
```
