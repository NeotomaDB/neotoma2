...
SIMON and SOCORRO - the following code does not work on my computer, but does work on Eric's.  Seems get_dataset call is not recognizing the site object as acceptable input.
...
The functions used to extract elements from the sites objects may not always have all the variables of interest, or too many variables. Custom tables, or data frames, can be made.  The following example creates a table containing siteid, sitename, collection unit id, datasetid, datasettype, age_range_old, age_range_ young, and first publication.

```{R custom_datasets_dataframe}
uwallf_site <- get_sites(sitename = 'Upper Wallface%')
#Error in data.frame(collectionunitid = x@collectionunitid, handle = x@handle,  : arguments imply differing number of rows: 1, 0
...
Use the `datasets()` function to get a simple table with a subset of basic dataset information.

###### `datasets` function
The `datasets()` function extracts datasets from a sites objects. 
...
#using the samples function to get the data output
diat_uwallf_data <- datasets(diat_uwallf_dataset)
#view the data frame of the data returned from the datasets function
diat_uwallf_data
...
# Example of how csv file could be saved to a specified directory:
# write.csv(diat_uwallf_data,"C:\\Users\\eco37\\Desktop\\datasets_uwallf.csv", row.names = FALSE)
```
...
##### `collunits` Function
The `collunits` function extracts collection units from a sites objects. 
...
#Save the output as a CSV file, which can be opened in Microsoft Excel.  The file will be saved in the working directory.
...
#### Diatom Counts
##### Samples Function
The `samples()` function obtains samples from a record or multiple records as a data frame. 

**Usage:** `samples(x)`

The argument for this function takes a "sites" object (x) obtained from `get_downloads`.
...
##### `taxa` Function
The `taxa` function extracts collection units from a sites objects. 
**Usage:** `taxa(object)`
The argument for this function takes a "sites" object.

**Values returned:** A datasets object displaying a dataframe with the variables units, context, element, taxonid, symmetry, taxongroup, elementtype, variablename (taxa), and ecologicalgroup.

```{R taxaFunctionExample}
#search for a site by site name
uwallf_site <- get_sites(sitename = 'Upper Wallface%')
#get datasets from get_sites output
uwallf_datasets <- get_datasets(uwallf_site)
...
uwallf_dwl <- get_downloads(diat_uwallf_dataset)
#using the samples function to get the data from the taxa function
diat_uwallf_taxa <- taxa(uwallf_dwl)
#view the collection unit data
diat_uwallf_taxa

#to save the output as a CSV file, which could be opened in Microsoft Excel
write.csv(diat_uwallf_taxa,"C:\\Users\\eco37\\Desktop\\taxa_uwallf.csv", row.names = FALSE)
```
