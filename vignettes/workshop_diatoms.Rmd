...
## Introduction
This document was prepared for use at workshops designed to introduce participants to use of the neotoma2 R package, primarily to browse, download, and analyze data in the Neotoma database.  It provides general guidance and specific code examples for those wanting to use `neotoma2` to retrieve and work with diatom data from the Neotoma Paleoecology Database. 
...
[[Provide more information on the functions - what they do.  get_sites and get_datasets are nearly the same. Can't use siteid or sitename with get_datasets. Get-downloads gets the data]]
...
The get_sites function can be used to search for information on a single or multiple sites. Sometimes it is helpful to find locations of all sites within a region, even if they are not diatom sites, as this indicates the level of paleoecological 'effort' in a region.
...
x is an optional value that represents the numeric dataset ID. [[[Why is it considered "optional"? Need more description of possibilities.  Can use outpu of any of 3 main functions?  Anything that is a sites object? DFC]]]

`get_datasets()` returns a **sites** object but with a more complex detail at the dataset level.  [[[Confirm this description!!]]]
]]]
