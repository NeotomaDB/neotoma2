...
The following functions can be used on a `sites` object to return associated values in a data frame without having to go through the structure of lists returned from `get_sites`, `get_datasets`, or `get_downloads`.

Some examples of helper functions are `datasets()`, `collunits()`, `samples()`,`taxa()`, and many others that are listed within the `neotoma2` package documentation. Below, we will show some basic examples of the output of these functions.
...
Use the `datasets()` function to get a simple table with a subset of basic dataset information. To convert this to a dataframe for manipulation use can use the following code:
...
Below is an example of getting sample data from multiple sites while also filtering for diatom and water chemistry datasets.
