...
```{R neotoma version, eval=FALSE}
interval_42 <- NL2 %>%
  neotoma2::filter(age_range_old > -42)
interval_42
```

```{r downloads after dplyr, eval=FALSE}
...
```{r datasettype and filter taxons, eval=FALSE}
...
```{R SitesbyConDBExample, eval=FALSE}
ConDB <- 
...
Example
```{R Diatom Aggregate Dataset Example, eval=FALSE}
```
...
```{R standard set, eval=FALSE}
get_dataset(taxonname = ,search())
```


```{R get_publicationsFunctionExample, eval=FALSE}
...
```{R Counts single dataset UWall EX, eval=FALSE}
library (neotoma)
...
```{R ExtractChroncontrol, eval=FALSE}
...
```{R Upper Wallface View counts, eval=FALSE}
...
``` {R TransformCountsToPct, eval=FALSE}
library ("analogue")
