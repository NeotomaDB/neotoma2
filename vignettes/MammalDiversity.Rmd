...
For our example, we want to search for all early Holocene vertebrate records within Neotoma in North America.  So we can construct our search as follows:

```{r getEasternFossils, eval=TRUE}
...
We can then validate the download, making sure we're getting the records we expect, by mapping them:
...
Once the data is filtered we can either continue to manage the data as sites, datasets or collection units, or we can begin to work directly with the raw sample data using the function `samples()`.  The `samples()` function returns information about the site, dataset, chronology and samples.  It tends to be very verbose, to support all of the user communities within the Neotoma data ecosystem:

```{r getsamples}
mamSamp <- samples(mamDl)
```

```{r mamSampDT, echo=TRUE}
DT::datatable(mamSamp[1:30,])
```
...
So, given the range of dates, lets clean things up so that we only accept dates with uncertainty less than 500yrs:

```{r getSamples}
mamClean <- mamSamp %>% 
  dplyr::filter(ageolder - ageyounger < 500)
```

You will note that `chronologyid` is one of the fields included in the `samples()` output.  We can use the chronologies to get finer-grained understanding of the temporal certainty for the records. Unfortunately, a large portion of the early mammal data (records submitted as part of Faunmap) within Neotoma is poorly constrained.  We can look at these records
...
From this data we then have a set of taxa:

```{r taxontable}
taxaTable <- mamClean %>% 
  group_by(variablename) %>% 
  summarise(n = length(unique(collunitid)))
```

```{r tabletable}
DT::datatable(taxaTable)
```

We can see that we're returning `r nrow(taxaTable)` taxa currently.  This may be more than we want.  In fact we can use the `taxaTable` to take a look at some of the taxa, or we can use the `taxa()` function on the `mamDl` object to get a more complete look at the taxon list present.  `taxaTable` shows us that `r sum(taxaTable$n==1)` taxa appear only once.

```{r mamRemoveTaxa}
mamClean <- mamClean %>% 
  dplyr::filter(variablename %in% taxaTable$variablename[taxaTable$n > 5], .preserve = FALSE)

taxaTable <- taxaTable %>% filter(n > 5)
```
...
We have filtered out the sets we are not interested in using, based on age, taxon or space.  For each taxon remaining we can use the `sf` package to create spatial polygons at two time periods, prior to 5000 cal yr BP and post 5000 cal yr BP:

```{r, makinghulls}

spatial <- mamClean %>% 
  mutate(agebin = ifelse(ageolder < 5000, "young", "old"))
...
for (i in 1:nrow(taxaTable)) {
  hulls[[i]] <- list()
  hulls[[i]]$variablename <- taxaTable$variablename[i]
  hulls[[i]]$hull <- spatial %>% 
    dplyr::filter(variablename == taxaTable$variablename[i]) %>% 
    sf::st_union() %>% 
    sf::st_convex_hull()
}
...
Download climate data from appropriate source. This is just an example of downloading one element of [WorldClim](https://www.worldclim.org/) data as a raster in R. This particular raster has `r raster::nlayers(worldTmin)` layers and is a global dataset.  The `raster` package for R has a number of links to external datasets such as WorldClim.  Alternately you can load in your own data.  In this case we are obtaining the minimum temperature for each month of the year, globally.

```{r, eval=TRUE}
worldTmin <- raster::getData('worldclim', var = 'tmin', res = 10)

raster::plot(worldTmin, 1)
values <- sf::st_crop(worldTmin, hulls[[1]]$hull)
```

Here we convert the raster to a `stars` object, for use with simple features.

### Join Hulls with Environmental Data
  
```{r hiddenrasterplot}
values <- st_crop(worldTmin, hulls[[1]]$hull)
plot(values[1])
```

Now we can extract the data for each of the convex hulls within the dataset.
