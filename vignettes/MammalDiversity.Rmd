...
```{r mamRemoveTaxa}
mamClean <- mamClean %>% 
  dplyr::filter(variablename %in% taxaTable$variablename[taxaTable$n > 9], .preserve = FALSE)

taxaTable <- taxaTable %>% filter(n > 9)
```
...
spatial <- st_as_sf(spatial, coords = c("long", "lat"))
...
for (i in 1:nrow(taxaTable)) {
  ...
  mutate(points = st_point(x = c(long, lat)))

  group_by(variablename, agebin) %>% 

  summarise(points = st_as_sf(data.frame(long = long, lat = lat)), coords = c("long", "lat"))
```
