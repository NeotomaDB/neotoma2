...
When you work with the `neotoma2` package you are working with data from the neotoma database, but it is mediated through R functions (like `get_sites()` to get site information) that themselves call public URLs, that are basically abstractions of database calls.  So, for example, to get data for site with ID number 12:
...
###### get_site
Usage: Sites can be searched using the following supported parameters, e.g., get_site("Sitename"):
...
To see structure of get_site function output, run the following code:
```{R structure_of_get_site_output}
uwallf_site <-get_site(sitename = 'Upper Wallface%') # finds site data for a site
...
To see structure of get_dataset function output, run the following code:
...
To see structure of get_download function output, run the following code:
...
###### get_sites
Needs much more description - arguments and values!!!!
-----
To get site info using site ID, just include the siteid, e.g., get_sites(475)
To get site info using sitename, use get_sites(sitename = "name of Site")
To see structure of get_sites function output, run the following code:
```{R structure_of_get_sites_output}
...
[Include brief explanation of differences between get_site (Neotoma R) and get_sites 9Neotoma R2).  `Neotoma2` R get_sites is working (totally?  altitude issues??) and provides more information than get_site.  Therefore, there is no reason to use get_site. [SIMON - is this true?]]
...
[Simon / Soccorro - the following code does not return the correct results; ir returns data for a different set of lakes - it seems to be getting sites by siteid and not site name]
...
# [Simon - Above code does not work - Will it work with only one siteid?]
```
...
# neDiatoms_tb <- get_dataset(datasettype="diatom top-bottom") # This code will not work because "diatom top-bottom" is not on the list of acceptable dataset types that neotoma R accesses.  It can be added to the code when neotoma2R code is changed to allow use of the Top-Bottom data type.
neDiatoms_dt <- get_dataset(datasettype="diatom")
# Combine results of search for three individual datatypes into one
neDiatoms_sstb <- c(neDiatoms_ss,neDiatoms_dt)
neDiatoms_sstb
