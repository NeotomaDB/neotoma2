...
```{r sitewithwildcardname, error=TRUE}
...
```{r agebounds, eval=FALSE, error=TRUE}
...
```{r getdatasetsbytype, error=TRUE}
...
```{r all_data, eval=FALSE, error=TRUE}
...
```{r filterAndShowTaxa, error=TRUE}
brazil <- '{"type": "Polygon", 
            "coordinates": [[
                [-73.125, -9.102],
                [-56.953, -33.138],
...
```{r pubsbyid, eval=FALSE, error=TRUE}
...
```{r showSinglePub, eval=FALSE, error=TRUE}
two[[2]]
```
...
```{r fulltestPubSearch, error=TRUE}
...
```{r getSecondPub, eval=FALSE, error=TRUE}
two[[1]]
```
...
```{r subsetPubs, eval=FALSE, error=TRUE}
...
```{r setNewPub, eval=FALSE, error=TRUE}
...
```{r setPubValue, eval=FALSE, error=TRUE}
