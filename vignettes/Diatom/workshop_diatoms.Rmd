...
Use the `get_sites()` or `get_datasets()` functions to find sites and related information. Searches using siteid or sitename can be done using `get_sites()`. Searches using dataset information (such as age, location, dataset type, etc.) can be done using `get_downloads()`. 
...
NEED CODE to transform raw diatom counts (number of rows counted) in a dataframe to percent relative abundance.  This could be done with using basic R code or by relying on a function in another package - for example the tran() function in the Analogue package.  The tran function can make a variety of other transformations as well.
