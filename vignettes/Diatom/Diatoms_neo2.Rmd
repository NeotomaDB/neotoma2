...
The above code is useful for obtaining data for pre-defined Aggregate Datasets.  Sets of Aggregate Dataset datasetids can be stored as .txt files and used as inputs to get_datasets.

Creates a dataframe with siteid, datasetid, and corresponding site information)(?) using datasetids.
...
Find datasets associated with a contactid.  [[Code below does not give correct response]]
...
Platt
#Find datasets associated with Platt (contactid ID 18).
...
# str(uwallf_data) # to see output, remove # from beginning of line
```
...
**Values returned:** A "publications" object that displays the variables publicationid, citation, and DOI. The structure of the "publications" object contains metadata with all possible publication information.
...
#find all istes in the northeast US
northeastUS <- rbind(c(-80,40), c(-80,48),
                     c(-65,48), c(-65,40), c(-80,40))
neus <- sf::st_polygon(list(northeastUS))
