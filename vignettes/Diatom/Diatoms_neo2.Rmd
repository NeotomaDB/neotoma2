...
The above code returns data for three sites.  We could use a larger set of site IDs either by loading the IDs from a file, or by entering them directly.  Here we assume that there exists a plain text file called `NLAD.txt` with a list of site IDs for sites sampled as part of the National Lakes Assessment.
...
```{R sample analyst}
# Finds 20 datasets with "diatom" datasettype
diatom_samples <- get_datasets(datasettype = "diatom") %>%
  get_downloads %>%
  samples()

# Search for specific sample analyst in datasets
