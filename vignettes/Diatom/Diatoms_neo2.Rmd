...
loc <- neotoma2::get_table('geopoliticalunits', limit=99999)
View(loc)
...
plains_data <- get_datasets(plains)
plains_ids <- as.data.frame(getids(plains_data))

plains_data_frame <- as.data.frame(plains_data)
plains_ids_frame <- as.data.frame(plains_ids)

final <- merge(plains_ids_frame, plains_data_frame) %>%
  arrange(., datasetid)
...
diat_uwallf_dataset <- neotoma2::filter(uwallf_data, datasettype == "diatom")
#take the diatom datasets and get the downloads output
...
[[Need code chunks for finding datasets that contain **one or more specific diatom taxa names, that are part of specific aggregate datasets**]]
...
[[Need example of getting a list of taxa from a download sites object - use result of an example above?]]
