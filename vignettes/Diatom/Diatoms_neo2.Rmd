...
Obtain data for multiple datasetids in pre-defined "Aggregate Datasets" files.  Sets of Aggregate Dataset datasetids can be stored as .txt or .csv files and used as inputs to get_datasets, either by pasting the datasetids into the code chunk or reading in the file.
...
#write.csv(final,"C:\\path\\to\\file_destination\\GreatPlains_agg.csv", row.names = FALSE)
...
# BELOW LINE DOESN'T RETURN ACCURATE RESULTS
...
The `chronologies()` function obtains chronology data from a record(s)[[sites object??]] and returns results as a list. 
...
**Values returned:** A "publications" object that displays the variables publicationid, citation, and DOI. The structure of the "publications" object contains all possible publication information.
