---
title: "Working with diatom data in the neotoma2 R-package - August 2022"
output: 
...
Note: There are four diatom dataset types:  "diatom" (stratigraphic / cores), "diatom surface sample" (surface samples only), "diatom bottom sample" (bottom samples only) and "diatom top-bottom" (top and bottom samples only).
...
Neotoma2 also includes helper functions such as `filter()`,`datasets()`, `collunits()`, `samples()`, and `plotLeaflet()`.  They are used to extract elements from the obtained `sites` objects.  
...
The above code returns data for three sites.  We could use a larger set of site IDs by either entering them directly or by loading the IDs from a file.  In the example below we assume that there exists a plain text file called `NLAD.txt` which contains a list of site IDs for sites sampled as part of the EPA National Lakes Assessment.

There may well be many more lines with numbers representing each site identifier in the set of records.  In this case we would load the data like so:
...
Example of finding sites by location within a bounding box of latitudes and longitudes.  Same code works with `get_datasets()` as well as `get_sites()`.
...
The code below will open the Geopolitical units table.  Once the table appears, use the search function at the top of the page to find the name of a geopolitical unit of interest and its corresponding gpid. 
...
Example - `get_sites()` using geopolitical ID for Alaska (6198), obtained from the GeoPoliticalUnits table.
...
Run following code to open the sitegeopolitical table and then enter siteid in filter box to find corresponding gpids
...
**Values returned:** A `sites` object with more detail than a `get_sites` object that includes information on collection units and datasets (e.g., datasettype, dataset notes and dataset age range).
...
Get all datasets for "diatom", "diatom top-bottom", "diatom surface sample." [Note: cannot currently (as of 22 Aug 2022) get datasets for "diatom bottom sample."]
...
Obtain data for multiple datasetids in pre-defined "Aggregate Dataset" files.  Sets of Aggregate Dataset datasetids can be stored as .txt or .csv files and used as inputs to get_datasets, either by pasting the datasetids into the code chunk or reading in the file.

The following code example creates a dataframe with siteid, datasetid, and corresponding site information for datasets in the Northern Great Plains aggregate dataset.
...
Find datasets for sites with samples representing a specific time interval.  Use the same code used to search by elevation range, above, but substitute "ageyoung" and "ageold" for "altmin" and "altmax".
...
Find datasets associated with a contactid.  [[Code below does not currently (August 2022) give correct response. It is being revised.]]
...
To see the structure of a `get_datasets()` object, run the following code:
...
Use the `get_downloads` function to download all records associated with one or more datasets.  The function returns a `sites` object with full level of detail.
...
**Values returned:** a `sites` object with a full level of detail, including data on sites, collection units, analysis units, samples, datasets, taxonomic data (e.g., diatom counts), chronologies, and contactid.
...
A typical next step is to create subsets of data for intended analyses.  The following helper functions can be used on a `sites` object to return a subset of associated values as a data frame without having to go through the structure of lists returned from `get_sites`, `get_datasets`, or `get_downloads`.

Some examples of helper functions are `datasets()`, `collunits()`, `samples()`,`taxa()`, `chronologies()` and `coordinates()`. These are also listed and described within the `neotoma2` package documentation.

#### `filter()` function

The filter function filters a sites object by latitude, longitude, elevation or dataset type.  It should be expressed in code as "neotoma2::filter ()" so it will not be confused with the dyplr's filter function.  See "neotoma2-package.Rmd" for more detailed description.  See the code chunk below (R datasetsFunctionExample) for an example.
...
The `collunits` function extracts collection unit information from a `sites` object. 

**Usage:** `collunits(object)`
The argument for this function takes a `sites` object.

**Values returned:** A datasets object with the variables datasetid, database, datasettype, age_range_old, age_range_young, and notes.

Example of using `collunits()` to obtain information on collection units for a site.
...
The `samples()` function obtains sample level data from a `sites` object created using get_downloads and returns results as a data frame. 
...
The `samples ()` function provides the most efficient way to obtain a data frame with diatom counts.  The following code chunks can be used to view diatom count related data.  There are a few examples, each with something a little different.
...
Example of using `samples()` to obtain diatom counts from multiple sediment cores.
...
Example of obtaining diatom counts for an aggregate dataset by reading an external file with datasetids.  To run code, replace "path to file" with a working path.
...
The `sites` objects obtained from the above two chunks of code can be combined to include both diatom count and water chemistry data.
...
The `chronologies()` function obtains chronology data from a sites object and returns results as a list. 
...
The `taxa ()` function obtains a list of unique taxa names contained in download sites object.

**Usage:** `taxa(x)`

**argument:** (x) is a `sites` object (that contains taxa names) obtained from `get_downloads`.

**values returned:** A data frame with 11 variables:  units, context, element, taxonid, symmetry, taxon group, element type, variable name, ecological group, sites - number in which the taxon occurs, and samples - number of samples in which the taxon occurs.
...
The `get_publications` function extracts publication information from a `sites` object. 
...
**Values returned:** A "publications" object and a dataframe that displays the variables publicationid, citation, and DOI. The "publications" object contains all possible publication information.
...
The map returned by `plotLeaflet()` is a `leaflet` object. Leaflet items (as defined in the `leaflet` package, e.g., custom markers) can then be added to the map as desired.
