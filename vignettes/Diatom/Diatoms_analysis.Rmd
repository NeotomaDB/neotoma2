...
The following code calculates percent abundance of diatoms from the count data contained in a sites object obtained using `get_downloads()`. A percent abundance column named `count_pct` is added to a dataframe containing other useful variables obtained from the `samples()` function.
...
The following code extracts state name using a site object
...
#function to add state names to samples () output dataframe
# x = geopol() output
...
Count data obtained from `samples()` is in a "long" format. Some common diatom analysis packages (such as Rioja) require the data be in a "wide" format . The following code shows how to transform wider.
...
Aggregate dataset work involves using the Excel workbook that contains all of the aggregate dataset information (e.g., dataset and sample ids of members of each aggregate set). 

As of 6/10/2022: The file is named Aggregate Datasets - Diatoms -  EMAP NE SW Neotoma (version 1) [[replace with current name]]

The file is located: \\files.drexel.edu\ANS\PCER_Data\Phycdata\PROJECTS\NEOTOMA\Aggreg Datasets - Dtm-WChem Corresp

The code chunks were initially made by just copying and pasting the datasetids into R. 
**EDIT**
...
Some of the sites objects are large. It may be worth saving them as an RDS file to be read in to avoid downtime.
...
# Scatterplot of percent abundance vs water chemistry
## Total pH against percent abundance - example
...
#LEAFLET
## Adding Leaflet Markers to existing map 

The following two code chunks are two custom leaflet function
...
all_sites <- rbind(diat_1_taxa, diat_4_taxa, diat_9_taxa, diat_15_taxa, diat_21_taxa, diat_22_taxa, diat_23_taxa, diat_24_taxa, diat_26_taxa,, diat_33_taxa, diat_34_taxa, diat_35_taxa, diat_36_taxa)
...
croto_abun_taxa_map_3_1 <- plotLeaflet_unclustered(diat_sites_all_3_1, df_with_count_pct = alltaxa_3_test_1, title = "Eunotia exigua % Abundance")

croto_abun_taxa_map_3 <- plotLeaflet_unclustered(diat_sites_all_3, df_with_count_pct = taxa_table_all_3, title = "Eunotia exigua % Abundance")
```
