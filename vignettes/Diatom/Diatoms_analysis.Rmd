...
The code chunk below was a working version. It needs to be reorganized / cleaned up. 

There should only be 1 plotting taxa (instead of 4) **DONE**. Makes use of a modified PlotLeaflet function in neotoma2 that I named PlotLeaflet2. This function is not in the document, but it is a separate script I have locally (add?). Extra code needs to be removed. Remove all variable naming (use %>%) **DONE I think**. 
...
filter(, grepl("stelligera", variablename, fixed = TRUE)) #used to see taxa with similar names

samp_stelligera <- samp_stelligera %>%
dplyr::filter(taxonid == 13088) #filtering again to only have get Discotella stelligera
...
# Standard Plot
## Plotting total Ph in WC against percent abundance
...
```{r RIOJA epd_binder strat example}
