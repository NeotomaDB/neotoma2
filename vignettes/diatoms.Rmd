...
[[[Jerrin - the following code creates a map - shouldn't this be a separate code chunk? maybe it should be placed after the next code chunk]]]
aa <- sitenameList %>% 
...
nla_sites <- c(nla_sites, sites)}  # ??Jerrin correct? - The object nla-sites now contains the get_sites results for all lakes listed in nlaxl
```
...
[[Jerrin - We should say something about where the .txt file is located]]
