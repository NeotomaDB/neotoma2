...
```{R genericGetDataset}
# Purpose of following line(s)
# diat_1 <- get_datasets(argument_1, argument_2)
# Additional explanations of code (optional)
```
...
# JERRIN / SIMON - Code above does not work using get_sites (Neotoma-r-2).  "Error in check_args(...) : Sitename should be a character"  Please try to fix.
```
...
# diat_el <- get_sites(`altmin()`, `altmax()`) - basic format
```
...
#  Where is "filename"saved? - What directory?
}
```
