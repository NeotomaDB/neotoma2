...
diat_1 <-get_site(sitename= 'Schrader Pond') # Example
```
### Site ID
```{r sitesbySiteID example}
diat_2<-get_site(siteid= x)
diat_3<-get_site(siteid= 27408) # Example
```
### Dataset ID

JERRIN - move code for searching by dataset ID to this location
### Dataset DOI
JERRIN - move code for searching by dataset DOI to this location
### Dataset Type
JERRIN - move code for searching by dataset Type to this location
...
### Dataset type - (e.g., Diatom Surface Sample)
     Diatom, Diatom Surface Sample, Diatom Top-Bottom
JERRIN - Shouldn't the DOI line of code below be deleted?  It is duplicated in the DOI section below.
...
```
### Dataset DOI [move to location above]
...
#### Site - Name(s) or ID(s) [Move to location above]
```{r sitesbyName example}
...
JERRIN - make the example search for keyword "modern."
```{R}
...
# Example similar to above but starting with a set of site ID's
```{R ExampleDataset}
library(purrr)
