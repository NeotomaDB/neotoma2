...
diat_1 <-get_site(sitename = "Name of site")  # Basic format
diat_2 <-get_site(sitename= 'Schrader Pond')  # Example
diat_1.5 <-get_site(sitename= '%Schrad%')  #Example using wildcards (%)

```
Get multiple sites by site names
```{r multsitesbySitename example}
library(neotoma)
# The get_site function (Neotoma R) can process only one lake at a time.  This code chunk allows retreiving data for multiple lakes.  It was created by Soccorro Dominguez.  The get_sites function being developed in Neotoma2 may allow processing multiple sites.
