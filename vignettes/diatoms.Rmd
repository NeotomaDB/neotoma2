...
uwallf_site <- get_sites(siteid = 745) # finds site data for a site
str (uwallf_site) # shows structure of uwallf_site
# [Simon - Above code does not work.  Can it be fixed?]
```
...
```
#### Chronology
One way to get chronology data is to first run get_download to create a download file and then run get_chroncontrol to extract the chronology data.  The example below gets chronology data from a download files with data for all sites in the northeast US.
```{R ExtractChroncontrol}
ne_chroncontrol <- get_chroncontrol(ne_down_dtsts, chronology = 1, verbose = TRUE, add = FALSE)

```
```{R ChronMetadata}
# Get chronology metadata
ne_chronmeta <-sapply (ne_chroncontrol, 
                 function(x) {               c(x$meta[7])
                 })
as.data.frame (ne_chronmeta)
str(ne_chronmeta)
view (ne_chronmeta)
```
                 #x$dataset$dataset.meta$dataset.id, #x$dataset$dataset.meta$dataset.name,  #x$dataset$dataset.meta$dataset.type, #x[[6]][2][4])
