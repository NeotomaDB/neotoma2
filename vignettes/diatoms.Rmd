...
#In the code it would get some metadata from Neotoma DB from the datasettype and location
```
### Site Name
Getting a single site with sitename 
```{r sitesbyName example}
diat_1 <-get_site(sitename = "Name of site")
diat_1.5 <-get_site(sitename= '%Schrader Pond%')
diat_2 <-get_site(sitename= 'Schrader Pond')# Example
#The code would be finding a site by its name and could use a wildcard to get an approximate of the sitename
```
...
# The multiple sitenames are combined into an object to be made in a for loop to run each name instead of doing it manually 
```

### Site ID
Getting a single site by using siteid
```{r sitesbySiteID example}
diat_3<-get_site(siteid= x)
diat_4<-get_site(siteid= 27408) # Example for single site 
#The get_site function will give basic metadata returning the sitename,location and collection unit 
```

Getting multiple sites by using siteids 
...
### Dataset ID
Getting a single dataset from a dataset id 
```{r get datasetID}
...
Example of getting information from a single site ID using get_dataset with subsets to return counts
```{R ExampleSingleSite }
Alexander<- get_dataset(24) 
subsetda <- sapply(Alexander, 
                 function(x) {
                   x$dataset.meta$dataset.id 
                   })
# In the subset it would run the dataset and search for the parameters by the use of dollar signs 
