...
[[Simon - Please provide reference for description and use of "limit" and "offset"; are these relevant just to retreivals from the Postgress database?]]
...
# JERRIN / SIMON - code seems to work up to this point, but the following line does not work. "Error in get_dataset.default(neDiatoms_sstb) : siteid must be numeric."
...
# [SIMON - This code chunk does not work.  Is the DOI function still being developed?
...
JERRIN / SIMON - code does not run - "Error in get_downloads(schrader.data) : 
  could not find function "get_downloads""
...
Code bein writtent for Neotoma 2
...
### Get DiatomBase data by taxaname

DiatomBase is a subset of WORMS - World Register of Marine Species
    https://www.diatombase.org/index.php
    http://www.marinespecies.org/index.php
    
DiatomBase R code is the same as that for WORRMS, but functions are limited to records of diatom taxa only

DiatomBase API's are at: https://www.diatombase.org/aphia.php?p=rest&__route__/
The following basic R code was copied on 29 Oct 2021 by DF Charles from: https://github.com/ropensci/worrms/blob/master/vignettes/worrms.Rmd
-----------
...
```
Notes: 
  The unique identifier for a taxon name in DiatomBase and Worms is called      "AphiaID"
...
Code chunks below 
  1.  Get AphiaID and taxonomic information by taxa names
  2.  Create an object that includes the AphiaID's only [not written]
  3.  Get source data, including original description, by AphiaID
  4.  Combine results of 1 and 3 above to get all necessary information to populate all fields in the Neotoma taxa list [not yet written]


by a taxonomic name
```{r DiatomBase1Taxon}
library("worrms")
dtmBase_taxon <- wm_records_name(name = 'Fragilaria acidobiontica', fuzzy = TRUE, marine_only = FALSE) 
# Set fuzzy = true (default) to include varieties, etc.; set = true to get specified name only)
...
```{r DiatomBaseTaxa}
library("worrms")
dtmBase_taxa <- wm_records_names(name = c('Navicula cryptocephala','Nitzschia palea', 'Navicula minima'), fuzzy = FALSE, marine_only = FALSE)
# Set fuzzy = true (default) to include varieties, etc.; set = true to get specified name only)
# Set marine-only as false; default value is true
dtmBase_out <- bind_rows(dtmBase_taxa)
view(dtmBase_out)
WriteXLS(dtmBase_out, ExcelFileName = "DtmBase_taxa_out.xlsx")
# Unfortunately, the function does not return the citation for the original description; code to do that follows
```
```{r DiatomBaseSources}
dtmBase_source <- wm_sources_(id = c(661528, 149203)) # AphiaIDs
