...
Example for further filtering with `dplyr::filter`
```{R}
interval_42 <- as.data.frame(datasets(NL2)) %>%
  dplyr::filter(age_range_old > -42)
interval_42
```
```{r downloads after dplyr}
get_downloads(interval_42$datasetid)
```
...
subdate <- get_datasets(subdate= "2021-09-23")
subdate
...
#** DON The code runs as if you had passed no arguments - it should return 25 items but give the warning that no adequate argument was given.
```
...
## More information would be appreciated (socorro)
```
...
```{r getWaterChem,}
neDiatoms <- get_datasets(datasettype="diatom surface sample", loc=BOUNDING_BOX)
```
