...
by a taxonomic name
```{r DiatomBase1Taxon}
dtmBase_taxon <- wm_records_name(name = 'Nitzschia minima', fuzzy = TRUE, marine_only = FALSE) 
...
# Set fuzzy = true (default) to include varieties, etc.; set = true to get specified name only)
# Set marine-only as false; default value is true
bind_rows(dtmBase_taxa)
view(dtmBase_taxa)
# Simon - this returns correct data, but output is not in a useful format; taxa results should appear under each other, not to the side.  Probably an easy fix for someone who knows what they are doing.
```
