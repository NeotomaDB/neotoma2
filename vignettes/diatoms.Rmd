...
#### NeotomaR and NeotomaR_2 functions
Following are descriptions of the main functions in Neotoma R and Neotoma2 R. See package documentation for more details.
...
##### Neotoma R functions
###### get_site
Sites can be searched using the following supported parameters, e.g., get_sites("Sitename"):
...
[[Provide reference for description and use of "limit" and "offset"]]

Values returned: A dataframe that includes: siteid, sitename, long [mean], lat [mean], elev, description, long_acc [width of bounding box], lat_acc [height of bounding box]
...
Valuses returned; "an object of class download_list, containing a set of download objects, each with relevant assemblage information and metadata: The download object is a list of lists and data frames that describe an assemblage, the constituent taxa, the chronology, site and PIs who contributed the data. The following are important components:" dataset, sample.meta, taxon list, counts, lab.data, chronologies"

##### Neotoma2 R functions
###### get_sites
###### get_datasets
###### get_downloads
...
#### Site ID
In the following code chunks, we assume that siteids are known.  They may have been obtained from using Neotoma Explorer, from Neotoma Landing Pages or from an external file.
...
```
#### Geographic location
Use get_site to return a list of all sites located in a geopolitical unit as specified by gpid
```{r get site by gpid}
...
write.csv(C2,file="C2.csv") # The file will be in the current working directory 
#The code above will not work if a value for altitude is null - as of Sept 16 2021 (Jerrin) 

```
Find sites where value for altitude are null
```{r }
