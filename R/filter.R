...
filter.sites <- function(.data, ..., .by = NULL, .preserve = FALSE) {
  ...
  sitecols <- c("sitename", "lat", "long", "altitude") %>%
    map(function(x) any(str_detect(ellipsis, x))) %>%
    unlist() %>%
    any()
  datasetcols <- c("datasetid", "database", "datasettype", "age_range_old",
                   "age_range_young", "notes", "recdatecreated",
                   "submission_date", "DOI") %>%
    map(function(x) any(str_detect(ellipsis, x))) %>%
    unlist() %>%
    any()
  ...
  if (datasetcols == TRUE) {
    ids <- ids %>%
      inner_join(as.data.frame(datasets(x)),
                 by = "datasetid")
  }
  cleanids <- ids %>%
    filter(...)
  ...
  pared_ds <- map(pared_sites@sites, function(x) {
    ycu <- collunits(x)
    ycu <- ycu[which(as.data.frame(ycu)$collectionunitid %in% good_cuid)]
    xcu <- map(ycu@collunits, function(y) {
      yds <- datasets(y)
